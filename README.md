...
- All data persists locally via `localStorage`. Some browsers (notably Firefox) block `localStorage` for files opened directly with `file://` — if that happens the app shows a warning and falls back to manual **Export/Import backup** (JSON file) so you don't lose progress
